...
export interface RoleContent {
   ...
   * ⚠ 처음엔 "앞으로 목소리와 목·어깨 쪽을 먼저 챙겨 드릴게요" 였는데 **무슨 말인지 모르겠다**는 지적을 받았다.
   *   🚫「챙겨 드릴게요」처럼 무엇이 달라지는지 안 말하는 표현을 쓰지 말 것 — **무엇을(몸풀기 제안) 어디를(부위)** 둘 다 적는다.
   *   그 아래 실제 제안 이름(`items`/`low`)을 예로 함께 보여준다 — 화면이 곧 확인이 된다.
   */
  picked: string;
  /**
   * 직군마다 다른 색. 2026-09-13 사용자 지시("3가지 카드의 구분선 컬러를 나눠줘")로 값만 넣어 뒀고
   * ⏸ **아직 어느 화면에도 안 붙었다**(그 작업 중 「정지」 지시를 받았다). 붙일 자리는 고른 카드 안 구분선과 그 아래 한 줄.
...
export const ROLES: Record<Role, RoleContent> = {
  teacher: {
    ...
    picked: "목·어깨·목소리를 풀어주는 몸풀기를 제안해 드려요.",
    accent: { line: "#7a6bc4", text: "#4a3f80" }, // 보라(브랜드)
    items: [
  ...
  admin: {
    ...
    picked: "눈·손목·허리를 풀어주는 몸풀기를 제안해 드려요.",
    accent: { line: "#4d7c8c", text: "#2d5c6e" }, // 청록
    items: [
  ...
  care: {
    ...
    picked: "손목·다리를 풀어주는 몸풀기를 제안해 드려요.",
    accent: { line: "#e0876c", text: "#8a4a3c" }, // 테라코타(신체 건강 타일과 같은 색)
    items: [
