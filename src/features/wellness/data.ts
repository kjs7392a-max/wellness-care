...
/**
 * 안전 확인에서 단계가 올라간 분께 하는 안내. **온보딩 판정 화면과 홈 제안 카드가 같은 문장을 쓴다**
 * — 두 곳에 따로 적으면 한쪽만 고쳐지는 날이 온다.
 *
 * 이력: 2026-09-13 「예」 1개와 7개가 같은 안내였다 → 개수 문턱(PARQ_TIER_AT) → 2026-09-16 문턱 삭제.
 *   단계는 safety-screen.ts 의 `safetyTier` 가 **문항 성격**으로 정한다(PAR-Q+ 는 약관상 못 써서 APSS·ACSM 기반으로 교체).
 *   ⚠ 아래 문구·PARQ_REST_ITEM 의 이름에 남은 "PARQ" 접두어는 식별자일 뿐 화면에 안 나온다.
 */

/**
 * 2단계(징후 「예」)에서 **직군과 무관하게** 내놓는 항목. 앉은 채로 숨만 고르는, 이 앱에서 가장 부하가 낮은 것.
...
/** 2단계 — 징후 문항에 「예」. 강도를 낮추는 것으로 끝내지 않고 무엇을 내놓는지까지 바꿨음을 말한다(🚫「여러 개」라고 쓰지 말 것 — 하나여도 2단계다). */
export const PARQ_REST_NOTICE = "안전 확인에서 몸 상태를 먼저 살펴야 할 항목이 있어요. 오늘은 몸에 부담이 없는 숨 고르기만 제안해 드려요.";
export const PARQ_REST_CONSULT = "운동을 시작하시기 전에 주치의와 꼭 먼저 상의해 주세요.";

// 2026-09-16: 문항 배열 PARQ 는 safety-screen.ts 의 SAFETY_QUESTIONS 로 옮겼다(문항마다 성격을 달아야 해서).
...
export const OB: ObStep[] = [
  ...
  { id: "promise", kicker: "", title: "이 앱은 선생님을 평가하지 않습니다", body: "점수나 순위를 매기지 않아요.", btn: "다음" },
  // 2026-09-12: 「동의」와 「권한」 페이지를 한 장으로 합침(사용자: "그렇게 많이 구분하면 귀찮아한다").
  // 2026-09-13: 「모으는 것은 이만큼이 전부예요」까지 같은 장으로 합침 → 2026-09-16: 그 목록과 본문은 삭제(사용자 지시). 이 장은 동의·권한만 받는다.
  { id: "consent", kicker: "동의·권한", title: "수집동의서 및 권한허용", body: "", btn: "동의하고 계속" },
  // 2026-09-12: 제목은 부드럽게, PAR-Q+ 는 표준 문항임을 앞세워 신뢰를 준다(사용자 지시).
  // 2026-09-13: 직군보다 앞으로 옮김(사용자 지시) — 안전 확인이 먼저 끝나야 직군별 제안의 강도가 정해진다.
  { id: "parq", kicker: "안전 확인", title: "몸 상태를 잠깐 여쭤볼게요", body: "국제 사전운동 문진을 참고한 아홉 가지 질문이에요. 해당하는 것이 있으면 그 부분만 조금 더 여쭤봅니다. 답에 따라 제안하는 강도만 달라지고, 몸에 무리가 가는 동작은 권하지 않습니다.", btn: "다음" },
