...
const OPENING: Record<Quadrant, string> = {
  HP: "오늘 고른 그림에는 앞으로 나아가려는 힘이 많이 담겨 있어요. 에너지가 올라와 있는 날이에요.",
  LP: "오늘은 마음이 꽤 차분한 날이에요. 고른 그림들이 대체로 편안하고 안정된 쪽에 있어요.",
  HN: "오늘 고른 그림에는 아직 가라앉지 않은 감정이 많이 담겨 있어요. 낮에 있었던 일이 마음속에서 계속 맴돌고 있어서, 몸도 같이 긴장해 있기 쉬워요.",
  LN: "오늘 고른 그림에는 쌓인 피로와 무거움이 많이 담겨 있어요. 힘이 빠져서 잠시 멈추고 싶은 마음이 커요.",
};

/** 둘째 문장 — 두 번째로 많은 결 하나를 짚어 준다(같은 사분면이면 생략) */
const SECOND: Record<Tone, string> = {
  활력: "그래도 움직이고 싶은 힘이 남아 있어요.",
  안정: "그래도 중심은 흔들리지 않고 있어요.",
  연결: "누군가와 함께하고 싶은 마음도 함께 있어요.",
  무게: "다만 피로가 조금 쌓여 있기도 해요.",
  정지: "동시에 잠깐 멈추고 싶은 마음도 있어요.",
  격동: "그리고 아직 가라앉지 않은 감정이 한쪽에 남아 있어요.",
};

const MIXED = "여섯 장의 선택이 여러 갈래로 나뉘었어요. 한 가지로 정해지지 않는 날도 괜찮아요. 오늘 하루에 여러 감정이 함께 있었다는 뜻이에요.";
