...
로그인 → 온보딩 4단계(3대 원칙 · 동의+권한 · 안전 확인 9문항+후속 · 직군) →
홈(AI 제안 · 날씨 · 2단 타일 · 오늘의 기록) / 기록(몸·마음) / 설정,
그리고 시트: 신체건강 라이브러리 · 타이머/가이드 영상 · 마음건강 · 마음과 대화 · 오늘의 마음카드.
...
- 모델: `claude-sonnet-5`.
- **데이터 출처(AI Hub 이용정책 표기 의무)**: 「마음과 대화」의 공감 배합 규칙·예시 3쌍은 한국지능정보사회진흥원(NIA) AI Hub **「공감형 대화」**(2022) 의 공감 유형 분포에서, 위험어 목록 보강은 AI Hub **「심리상담 데이터」**(2024)·**「웰니스 대화 스크립트 데이터셋」** 발화와의 대조에서 도출했습니다. 데이터 원문은 앱·저장소에 포함하지 않으며(재배포 금지), 회귀 시험지는 로컬 `.bench/`(gitignore) 에만 둡니다.
